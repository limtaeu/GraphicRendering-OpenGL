...
// Note that numPixelsOnWidth must be a even number.
{
	...
	double widthPerPixel = 2.0 / numPixelsOnWidth;
	double heightPerPixel = widthPerPixel;
	int numPixelsOnHeight = numPixelsOnWidth;

	double startX = -1 + widthPerPixel / 2.0;
	double startY = -1 + widthPerPixel / 2.0;

	for (int r = 0; r < numPixelsOnHeight; r++) {
		y = startY + (double)r * heightPerPixel;

		for (int c = 0; c < numPixelsOnWidth; c++) {
			x = startX + (double)c * widthPerPixel;
			deltaFormFactors[numPixelsOnWidth*r + c] = da / (M_PI * pow(x*x + y*y + 1, 2));
		}
	}
...
// Note that numPixelsOnWidth must be a even number.
{
	...
	double widthPerPixel = 2.0 / numPixelsOnWidth;
	double heightPerPixel = widthPerPixel;
	int numPixelsOnHeight = numPixelsOnWidth / 2;

	double startY = 1 - widthPerPixel / 2.0;
	double startZ = 0 + widthPerPixel / 2.0;

	for (int r = 0; r < numPixelsOnHeight; r++) {
		z = startZ + (double)r *  heightPerPixel;

		for (int c = 0; c < numPixelsOnWidth; c++) {
			y = startY - (double)c * widthPerPixel;
			deltaFormFactors[numPixelsOnWidth * r + c] = da * z / (M_PI * pow(y*y + z*z + 1, 2));
		}
	}
...
// Need to set up the viewport, projection and view transfromation.
{
	...
	VecNormalize(sideVecN, sideVec);
	VecCrossProd(lookUpVec, sideVecN, shooterQuad->normal);
...
static void ComputeRadiosity(void)
{
	...
	for (int iterationCount = 0; iterationCount < maxIterations; iterationCount++)
	{
		...
		if (firstShot) {
			prevUnshotPowerSum += unshotPower[0] + unshotPower[1] + unshotPower[2];
			firstShot = false;
		}
		else {
			float sum = 0;
			sum += unshotPower[0] + unshotPower[1] + unshotPower[2];


			if ((sum / prevUnshotPowerSum) < 0.004f) {
				break;
			}

			
			if (sum > prevUnshotPowerSum) {
				prevUnshotPowerSum = sum;
			}
			
		}


		/*if (VecLen(unshotPower) < 2000.0f) {
