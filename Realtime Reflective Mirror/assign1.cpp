...
const char fabricTexFile[] = "images/fabric.jpg";
...
void MakeReflectionImage( void )
{
	...
	glLoadIdentity();
	glFrustum(-1.5 - eyePos[1], 1.5 - eyePos[1], -1 - eyePos[0], 1 - eyePos[0], -1 * (1.2 - eyePos[2]), 100);

	// STEP 3: Sets up the imaginary viewpoint.
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();

	float newEyeZ = 2 * TABLETOP_Z - eyePos[2];
	gluLookAt(eyePos[0], eyePos[1], newEyeZ, eyePos[0], eyePos[1], eyePos[2], 1.0, 0.0, 0.0);
...
// This texture object is for the spots texture map.
	...
	glGenTextures(1, &fabricTexObj);
	glBindTexture(GL_TEXTURE_2D, fabricTexObj);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

	if (ReadImageFile(fabricTexFile, &imageData,
		&imageWidth, &imageHeight, &numComponents) == 0) exit(1);
	if (numComponents != 3)
	{
		fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
void DrawPlane(void) {
	...
	glBindTexture(GL_TEXTURE_2D, fabricTexObj); // Texture object ID == 0 means no texture mapping.

	glNormal3f(0.0, 0.0, 1.0); // Normal vector.
	SubdivideAndDrawQuad(24, 24, 0.0, 0.0, -size, -size, 0,
		0.0, 1.0, size, -size, 0,
		1.0, 1.0, size, size, 0,
		1.0, 0.0, -size, size, 0);
