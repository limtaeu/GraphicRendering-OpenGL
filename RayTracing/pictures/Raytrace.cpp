...
// Compute shadow if hasShadow is true.
	for (int i = 0; i < scene.numPtLights; i++) {
		Vector3d LDist = scene.ptLight[i].position - nearestHitRec.p;
		Vector3d L = LDist.unitVector();
		bool isShadow = false;
		Ray LRay(nearestHitRec.p, L);

		if (hasShadow) {
			for (int i = 0; i < scene.numSurfaces; i++)
			{
				bool hasHit = scene.surfacep[i]->shadowHit(LRay, DEFAULT_TMIN, LDist.length());
	...
	Vector3d newRayDir = mirrorReflect(V, N);
	Ray reflecRay(nearestHitRec.p, newRayDir);

	if (reflectLevels != 0) {
		return result + nearestHitRec.mat_ptr->k_rg * TraceRay(reflecRay, scene, reflectLevels - 1, hasShadow);
	}
	else {
		return result;
	}
